...
const basePath = '/var/www/html/data/';
...
// https://www.villes-voisines.fr/getcp.php?cp=38500&rayon=15
let villes = Object.values(JSON.parse(fs.readFileSync(`${basePath}villes.json`))).map(v => {
    return v.nom_commune.toUpperCase().replace('ST','SAINT').replace(/ /g, '-').normalize("NFD").replace(/[\u0300-\u036f]/g, "");
});
...
try {
    oldAnnonces = JSON.parse(fs.readFileSync(`${basePath}annonces.json`))
        .filter(a => villes.includes(a.ville.toUpperCase().replace('ST','SAINT').replace(/ /g, '-').normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim()));
